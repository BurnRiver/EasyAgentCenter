...
function getLatestUsageLines(lines: string[]): string[] {
  const commandStart = findLastLineIndex(lines, /^\/usage\b/i)
  const totalCostStart = findLastLineIndex(lines, /total\s+cost\s*:/i)
  const usageStart = Math.max(commandStart, totalCostStart)
  return usageStart >= 0 ? lines.slice(usageStart) : lines
}
...
function parseUsageMetric(lines: string[], scope: 'fiveHour' | 'sevenDay'): UsageMetric {
  const scopePattern = scope === 'fiveHour'
    ? /(5\s*(?:h|hr|hour|hours)|five\s*hour|5小时|五小时)/i
    : /(7\s*(?:d|day|days)|seven\s*day|week|weekly|7天|七天|本周|周)/i
  const metric: UsageMetric = {}

  for (const line of lines.filter((item) => scopePattern.test(item))) {
    const remainingPercent = parsePercentFromLine(line)
...
function parseSessionStats(lines: string[]): SessionStat[] {
  const totalCost = firstMatch(lines, /total\s+cost\s*:\s*\$?\s*([\d,.]+)/i)
  const apiDuration = firstMatch(lines, /total\s+duration\s*\(api\)\s*:\s*(.+)$/i)
  const wallDuration = firstMatch(lines, /total\s+duration\s*\(wall\)\s*:\s*(.+)$/i)
  const tokenTotal = firstMatch(lines, /total\s+tokens?\s*:\s*([\d,.]+)/i)
  const codeChanges = firstMatch(lines, /total\s+code\s+changes\s*:\s*(.+)$/i)

  return [
    totalCost ? { labelKey: 'claudeUsage.totalCost', value: formatCost(totalCost) } : null,
    tokenTotal ? { labelKey: 'claudeUsage.totalTokens', value: tokenTotal } : null,
    apiDuration ? { labelKey: 'claudeUsage.apiDuration', value: apiDuration } : null,
    wallDuration ? { labelKey: 'claudeUsage.wallDuration', value: wallDuration } : null,
    codeChanges ? { labelKey: 'claudeUsage.codeChanges', value: codeChanges } : null,
  ].filter((item): item is SessionStat => Boolean(item))
}

function parseClaudeUsage(output: string): ParsedClaudeUsage {
  const lines = normalizeOutput(output)
  const usageLines = getLatestUsageLines(lines)
  const summaryLines = usageLines
    .filter((line) => /(usage|limit|rate|reset|token|context|cost|duration|plan|activity|breakdown|5\s*h|5\s*hour|7\s*day|week|weekly|用量|额度|限制|重置|令牌|成本|费用|上下文)/i.test(line))
    .slice(0, 12)

  return {
    fiveHour: parseUsageMetric(usageLines, 'fiveHour'),
    sevenDay: parseUsageMetric(usageLines, 'sevenDay'),
    stats: parseSessionStats(usageLines),
    summaryLines,
  }
}
...
export default function ClaudeUsagePanel({
  ...
  const { t } = useI18n()
  const parsed = useMemo(() => parseClaudeUsage(output), [output])

  if (!session || session.agentId !== 'claude') return null

  return (
    <div className="quota-panel">
      <div className="quota-header">
        <h3>{t('claudeUsage.title')}</h3>
      ...
      {enabled && (
        <>
          <div className="quota-grid">
            <UsageMeter label={t('claudeUsage.fiveHour')} metric={parsed.fiveHour} />
            <UsageMeter label={t('claudeUsage.sevenDay')} metric={parsed.sevenDay} />
          </div>

          {parsed.stats.length > 0 && (
            <div className="usage-stat-grid">
              {parsed.stats.map((stat) => (
                <div className="usage-stat" key={stat.labelKey}>
          ...
          <div className="quota-hint">{t('claudeUsage.hint')}</div>
          ...
          {parsed.summaryLines.length > 0 ? (
            ...
            <div className="quota-empty">{t('claudeUsage.empty')}</div>
          )}
        </>
      )}
    </div>
